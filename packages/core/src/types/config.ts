import type { OpenAPIObject, SchemaObject, ReferenceObject, PathItemObject, ParameterObject } from "openapi3-ts/oas31";
import type { ApiCtx, ApiCtxConfig } from "./api";

export type FileCodeType = string[] | ((filename: string, ...args: any[]) => string[]);

export interface ConfigDtsOptions {
  /**
   * 是否启用TS类型
   * @default true
