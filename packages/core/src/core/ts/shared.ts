import type { ConfigDtsOptions } from "@/types";
 ...
 * @returns 类型名
 */
export const resolveRefTypeName = (ref: string, prefix?: string): string => {
  const name =
    ref
      .split("/")
      .pop()
...
const objectToTsType = (
  ...
  if (hasProps) {
    ...
    if (hasAdditionalProps && schema.additionalProperties !== false) {
      const addType =
        schema.additionalProperties === true ? "any" : schemaToTs(schema.additionalProperties as SchemaObject | ReferenceObject, prefix);
 ...
 */
export const createSchemaToTs = (
  typeConvert?: ConfigDtsOptions["typeConvert"],
): ((schemaJson: SchemaObject | ReferenceObject, prefix?: string) => string) => {
