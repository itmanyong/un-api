import type { ApiCtxConfig, ConfigDocOptions, ApiCtx, RecordType } from "@/types";
...
const resolveContentTypeName = (contentType: string): string => {
  return CONTENT_TYPE_MAP[contentType as keyof typeof CONTENT_TYPE_MAP] ?? contentType.split(/[^a-zA-Z0-9]/).at(-1) ?? "";
};
 ...
 * 根据类型值获取接口params参数
 */
export const getApiParamList = (paramIn: ParameterLocation, apiConfig: ApiCtx, apiDoc: OpenAPIObject): OasParameterObject[] => {
 ...
 * 获取接口的body参数
 */
export const getApiBodyJson = (contentType: string, apiConfig: ApiCtx, apiDoc: OpenAPIObject): SchemaObject | null => {
 ...
 * 生成 API 配置
 */
export const generateApiConfig = (ctx: ApiCtx): ApiCtxConfig => {
...
const processApiConfig = (
  ...
  moduleMap: Map<string, Required<ApiCtx>[]>,
): void => {
  ...
  if (docConfig.apiParser) {
    apiConfig.config = merge(apiConfig.config, docConfig.apiParser(apiConfig) ?? ({} as ApiCtxConfig));
  }

  let module = apiConfig.config?.module || "all";
  if (module === "index") module = "all";
  if (!moduleMap.has(module)) moduleMap.set(module, []);
  moduleMap.get(module)?.push(apiConfig as Required<ApiCtx>);
};

/**
 * 将文档接口扁平化转化为模块-接口列表的映射
 */
export const flattenApiMap = (apiDoc: OpenAPIObject, docConfig: ConfigDocOptions): Map<string, Required<ApiCtx>[]> => {
  const moduleMap = new Map<string, Required<ApiCtx>[]>();
