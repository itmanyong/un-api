 ...
 * 获取单接口注释
 */
export const getApiNote = (apiConfig: Required<ApiCtx>): string => {
  const { url, method, module, summary, name } = apiConfig.config ?? {};
  return [`/**`, ` * ${TABLE_COLUMNS}`, ` * ${TABLE_HEADER}`, ` * ${buildApiRow(module, name, url, method, summary)}`, ` */`].join("\n");
};

/**
 * 获取单接口注释行（无包裹语法）
 */
export const getApiNoteRow = (apiConfig: Required<ApiCtx>): string => {
  const { url, method, module, summary, name } = apiConfig.config ?? {};
 ...
 * 获取单个模块注释
 */
export const getModuleNote = (moduleName: string, apiModuleList: Required<ApiCtx>[]): string => {
  const rows = apiModuleList.map((apiConfig) => getApiNoteRow(apiConfig));
  return [`/**`, ` * ${TABLE_COLUMNS}`, ` * ${TABLE_HEADER}`, ` * ${rows.join("\n * ")}`, ` */`].join("\n");
};

/**
 * 获取单个模块注释行（无包裹语法）
 */
export const getModuleNoteRows = (moduleName: string, apiModuleList: Required<ApiCtx>[]): string => {
 ...
 * 获取文档注释
 */
export const getDocNote = (moduleMap: Map<string, Required<ApiCtx>[]>): string => {
  const rows: string[] = [];
 ...
 */
export const getFieldNote = (fieldName: string, schema: SchemaObject | ReferenceObject, required: boolean, globalPrefix?: string): string => {
  if ("$ref" in schema) {
 ...
 */
export const getTypedField = (fieldName: string, schema: SchemaObject | ReferenceObject, required: boolean, indent: string = "  "): string => {
 ...
 * 判断参数项是否必填
 */
export const isParamsRequired = (apiConfig: ApiCtx, type: keyof ApiCtx): boolean => {
...
export const getTypedParams = (
  ...
  // 考虑schema是引用类型的时候需要将其扁平化一层
  const props = params.reduce(
    (acc, param) => {
      if (param.schema && "$ref" in param.schema) {
        const refName = param.schema.$ref?.split("/").pop() ?? "any";
        const fieldType = globalPrefix ? `${globalPrefix}.${refName}` : refName;
        acc.refList.push(fieldType);
        return acc;
      }
      ...
      acc.typeList.push(`${indent}${note}\n${indent}${safeKey}${required}: ${fieldType};`);
      return acc;
    },
    { refList: [], typeList: [] } as { refList: string[]; typeList: string[] },
  );
  ...
  if (props.refList?.length) {
    result += (result ? "&" : "") + props.refList.join(" & ");
  }
